...
const useCarStatusPoller = (cars, setCars) => {
  ...
  const pollCarStatuses = useCallback(async () => {
    try {
      ...
      const response = await axios.get('http://meistermode-carsharing-backend-7481.twc1.net:8000/api/cars/status', {
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      ...
      try {
        const response = await axios.get('http://meistermode-carsharing-backend-7481.twc1.net:8000/api/bookings', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
