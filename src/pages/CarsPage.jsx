...
const CarsPage = () => {
  ...
  const handleBooking = async (carId, startTime, endTime) => {
    try {
      const response = await axios.post('https://meistermode-carsharing-backend-7481.twc1.net:8000/api/bookings', {
        car_id: carId,
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        ...
        setIsLoading(true);
        const response = await axios.get('https://meistermode-carsharing-backend-7481.twc1.net:8000/api/cars', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
